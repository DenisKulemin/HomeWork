...
// print_dict_with_loop and print_dict_with_iter replace previos function print_dict
// multiple_delete_words_from_dict was added

using namespace std;

void add_word_to_dict(multimap<wstring, wstring> &dict, wstring key = L"", wstring value = L"");

void print_dict_with_loop(multimap<wstring, wstring> &dict);

void print_dict_with_iter(multimap<wstring, wstring> &dict);

void delete_word_from_dict(multimap<wstring, wstring> &dict, wstring key = L"", wstring value = L"");

void multiple_delete_words_from_dict(multimap<wstring, wstring> &dict);
...
int main()
{
    ...
    while (true) {
        ...
        wcout << L"m - delete pairs from the dictionary from one letter to another." << endl;
        ...
        switch (answer)
            ...
            case L'd':
                ...
                break;
            case L'm':
                multiple_delete_words_from_dict(en_ru_dict);
                break;
            case L'p':
                print_one_word_from_dict(en_ru_dict);
                break;
            case L's':
                print_dict_with_iter(en_ru_dict);
                print_dict_with_loop(en_ru_dict);
                break;
            default:
...
void print_dict_with_loop(multimap<wstring, wstring> &dict) {
    wcout << L"Print all pairs from the current dictionary use range for loop." << endl;
    for (auto el : dict) {
...
void print_dict_with_iter(multimap<wstring, wstring> &dict) {
    wcout << L"Print all pairs from the current dictionary use iterator." << endl;
...
void multiple_delete_words_from_dict(multimap<wstring, wstring> &dict) {
    wchar_t start_letter = L'a';
    wcout << L"Enter the letter starting with which you want to remove words from the dictionary: ";
    wcin.get(start_letter);
    wcin.ignore(10000, L'\n');
    wchar_t end_letter = L'z';
    wcout << L"Enter the letter ending with which you want to complete the removal of words from the dictionary: ";
    wcin.get(end_letter);
    wcin.ignore(10000, L'\n');

    for (auto el : dict) {
        if (start_letter <= el.first[0] and el.first[0] <= end_letter) {
            dict.erase(el.first);
        }
        else if (el.first[0] > end_letter) {
            break;
        }
    }
}
